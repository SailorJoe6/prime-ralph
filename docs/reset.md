...
`/reset` is a project-local Prime Agent command. It resets only model-visible messages in the existing session and re-enters the plugin-owned specification or planning phase. It does not call `newSession`, session switching, forking, tree navigation, goal, autonomous, or REPL lifecycle APIs. It uses Prime Agent's public custom-compaction API when the host considers the current branch eligible.
...
Prime Agent reconstructs an empty compaction as a fixed summary wrapper. The `context` hook removes only a versioned Ralph empty wrapper. Prime Agent invokes that hook before it appends the current custom prompt to the provider context, so the first reset transformation projects one in-memory copy of the exact pending prepare message and consumes that one-shot guard. The durable message is still written once to JSONL. Lifecycle admission callbacks run only when the pre-provider context gate reaches exactly one correlated message matching durable `prepare_pending` evidence; unrelated queued provider contexts remain untouched until then; callback, correlation, or state-append failure aborts the provider request. Prime Agent persists the custom message itself asynchronously, so an interrupted pending transaction is never replayed. Later transformations project from the newest persisted prepare message. That persisted-boundary projection is required as an exactness guard because Prime Agent can restore completed host-managed IPython turn structures around a compaction. It also keeps later tool continuations while excluding every message before prepare.
...
`npm test` covers strict YAML/UTF-8/size and specification/planning prompt-version validation, exact active-path conflicts, ordered phase-specific reset injection, real marker matching, custom compaction results, wrapper filtering, exact prepare projection, short fallback, duplicate handling, ordinary-compaction isolation, synchronous admission failure, provider error state, reload recovery, and shutdown interruption.
...
Specification reset behavior is documented in [`specification.md`](specification.md). Planning reset behavior is documented in [`planning.md`](planning.md). Both use this same boundary transaction, single-flight guard, failure handling, and exact context projection. The combined message always places `prepare` before the selected phase skill and never starts execution.
