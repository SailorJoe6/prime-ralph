...
Prime Agent's public extension API exposes fire-and-forget `ctx.compact()` and a void `sendMessage()` without a delivery receipt. Prime Ralph therefore keeps every prepared boundary unacknowledged until the exact request reaches the pre-provider `context` hook. A bounded watchdog reconciles a still-unadmitted `origin_ending` or `prepare_pending` request only after the host is idle, has no pending actions, and has no exact durable dispatch record. Accepted queued work is never expired merely because the watchdog deadline passed. After a rejected queue drains, the next `input` hook provides the provider-free reconciliation boundary; `session_start` provides the reload path. Either path records terminal `failed`, invokes the rejection callback once, clears in-memory ownership, and admits no provider request or cycle. Prime Ralph prefers fail-closed uncertainty over replay; it does not modify Prime Agent or infer completion of host-unobservable detached work.
