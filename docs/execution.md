...
Short-session refusal, an already-compacted branch, cancellation, failure, interruption, or ambiguous recovery admits no next cycle and pauses safely. Prime Agent `0.9.3` checks short/already-compacted conditions before the extension compaction hook, so the public extension API cannot force those cases. Ralph deliberately does not restore automation with a projection fallback.
