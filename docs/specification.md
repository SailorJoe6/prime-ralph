...
2. `npm run accept:specification` uses a deterministic provider and real Prime Agent `0.9.3` session lifecycle. It proves startup is the first provider turn, reload does not replay it, `/spec-it-out` preserves conversation, reset excludes stale conversation, and no later workflow state starts. Initialization acceptance separately queries Prime Agent's real command catalog and rejects direct `skill:<canonical-name>` duplicates.
