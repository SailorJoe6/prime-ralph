...
Tracked RLM work can end one Agent run and deliver its terminal message in another while the same native goal remains active. Ralph re-registers every reconciled running execution at `before_agent_start`, so the later normal `turn_end` still owns lifecycle closeout. Prime Agent `0.9.3` automatic provider retry is a narrower exception: its direct `agent.continue()` emits `agent_start` without `before_agent_start`. Ralph reclaims ownership there only when same-goal reconciliation has restored the exact lifecycle, cycle, and driver from the durable `execution agent ended without normal closeout` pause. A changed, cleared, errored, stale, or append-failed retry is aborted before provider admission and remains durably paused; an exact paused or budget-limited driver stays fenced until it resumes. The closeout fence also remembers a matching event that settled before waiter registration. If neither ordering produces a durable closeout within 30 seconds, or the Agent run ends or the session shuts down, Ralph releases the waiter and pauses instead of leaving provider admission hung indefinitely.
