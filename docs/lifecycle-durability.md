...
Correlation requires the exact session, Ralph lifecycle, cycle, native goal, continuation count, boundary identity, and request ID before consumption or suppression. The workflow now recognizes only the matching namespaced empty compaction summary for an already-armed transaction. If steering or a tool tail selects that provider path before the queued execution message starts, Ralph persists `projection-consumed` before returning the reconstructed execution boundary. Repeated provider projection, including projection after extension reconstruction, is idempotent. The later exact queued message is aborted from the durable record even after extension reconstruction, without letting the deliberately aborted host run pause the lifecycle. A failed consumption append admits no projected pass and leaves the armed queued message authoritative. Any post-summary execution message must be the single exact correlated boundary; a near-match or duplicate candidate fails closed instead of exposing two execution authorities to the provider.
