...
test("execute templates define continue, wait, block, and complete semantics", () => {
  for (const variant of ["default", "beads"]) {
    const text = load(variant, "execute");
    for (const pattern of [/single highest-value task/i, /\*\*Continue:\*\*/, /\*\*Wait:\*\*/, /same Ralph lifecycle/i, /durable compaction transaction/i, /evidence that establishes readiness/i, /successful `ready` call.*end the assistant turn immediately/is, /`recover-driver`.*does not log or advance the cycle/is, /\*\*Blocked:\*\*/, /exact active pair without overwrite/i, /Stop\/delete any agent-owned wakeup/i, /tell the user what help is required/i, /\*\*Complete:\*\*/, /Audit every specification requirement/i, /native driver cannot continue/i, /native-goal action and Ralph lifecycle decision as one ordered protocol/i, /matching `ralph_lifecycle` call MUST be your next action/i, /finish the already-started lifecycle decision.*before inspecting or acting on unrelated queued content/is, /Do not repeat `goal\.complete\(\)`/i, /final response must identify/i]) assert.match(text, pattern);
