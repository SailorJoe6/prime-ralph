...
const result = { calls, providerTranscriptEquivalent: transcriptMatches.length === contexts.length && transcriptMatches.every(Boolean), resetPrepareCount: resetText.split(prepare).length - 1, staleExcluded: !resetText.includes(busySentinel) && !resetText.includes(queuedSentinel), resetBoundaries: entries.filter((entry) => entry.type === "custom_message" && entry.customType === "prime_ralph_reset_prepare").length, completedStates: entries.filter((entry) => entry.type === "custom" && entry.customType === "prime_ralph_reset_state" && entry.data?.status === "completed").length, resetModes: entries.filter((entry) => entry.type === "custom_message" && entry.customType === "prime_ralph_reset_prepare").map((entry) => entry.details?.mode), resetCompactions: entries.filter((entry) => entry.type === "compaction" && entry.customInstructions?.startsWith("prime-ralph-reset:v3:")).length };
