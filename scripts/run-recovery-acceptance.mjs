...
async function fixture({ cancel = false, legacy = false } = {}) {
...
`);
  ...
  return { cwd, session, sm, provisioner, sessionId, sessionFile, anchorId, priorLeafId, beforeBytes, afterBytes, entryCount, providerCalls: () => providerCalls, specPath, planPath, specBefore, planBefore, mutationPath, names, sentinel, preflight, commandSurface, notices };
}
...
await success.session.disposeAsync({ kernelSnapshot: false }); await legacy.session.disposeAsync({ kernelSnapshot: false }); await cancelled.session.disposeAsync({ kernelSnapshot: false });
const checks = { ...successChecks, ...legacyChecks, ...cancelChecks }; const failures = Object.entries(checks).filter(([, value]) => value !== true).map(([key]) => key);
