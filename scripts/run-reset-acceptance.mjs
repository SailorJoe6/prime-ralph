...
const responses = [
  ...
  () => assistant([{ type: "text", text: "second reset complete" }]),
];
let hostSession;
const agent = new Agent({
  ...
  streamFn: async (_model, context) => { contexts.push({ systemPrompt: context.systemPrompt, messages: structuredClone(context.messages) }); const fn = responses[call++]; if (!fn) throw new Error(`unexpected provider call ${call}`); return response(fn()); },
  sessionId,
});
...
const resetContexts = [contexts[5], contexts[7]];
...
const result = {
  ...
  prepareOnlyInitialContexts: resetContexts.every((context) => context.messages.length === 1 && context.messages[0]?.role === "user"),
...
if (result.providerCalls !== 8) failures.push("unexpected provider call count");
if (!result.prepareCounts.every((count) => count === 1)) failures.push("prepare not injected exactly once per reset");
if (!result.prepareOnlyInitialContexts) failures.push("reset provider context contained messages before prepare");
...
if (result.resetMessages !== 2) failures.push("two settled resets did not produce two boundaries");
if (result.resetCompactions !== 1 || JSON.stringify(result.resetModes) !== JSON.stringify(["compaction", "projection-fallback"])) failures.push("compaction and short-session fallback modes were not both exercised");
