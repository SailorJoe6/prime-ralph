...
const extension = (pi) => {
  ...
  pi.on("session_compact", (event) => trace.push({ hook: "session_compact", summary: event.compactionEntry.summary, fromExtension: event.fromExtension }));
};
...
const session = new AgentSession({ agent, sessionManager: sm, settingsManager: settings, cwd, agentDir, resourceLoader: loader, modelRegistry: registry, includeGoals: true, includeCompactSkill: true, initialGoal: { objective: "POC goal" } });
...
const postCompactionGoal = entries.some((entry) => entry.type === "custom" && entry.customType === "goal_context" && compaction && entries.indexOf(entry) > entries.indexOf(compaction));
const result = { cwd, calls, trace, entries: entries.map((e) => ({ type: e.type, customType: e.customType, summary: e.summary, firstKeptEntryId: e.firstKeptEntryId })), compactionApplied: Boolean(compaction), postCompactionGoalContext: postCompactionGoal };
if (!compaction || calls !== 1 || postCompactionGoal) {
