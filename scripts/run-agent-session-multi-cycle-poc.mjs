...
const responseFor = () => {
  const text = calls++ === 0 ? "normal response" : "continuation response";
...
const extension = (pi) => {
  ...
  pi.on("turn_end", (event, ctx) => {
    ...
    if (compactions < 2 && event.message?.role === "assistant" && !event.message.content?.some((part) => part.type === "toolCall")) {
      compactions += 1;
  ...
  pi.on("session_compact", (event) => {
    ...
    if (compactions < 2) setTimeout(() => pi.sendMessage({ customType: "goal_context", content: "<goal_context>continue</goal_context>", display: true, details: { source: "prime-ralph-poc" } }, { triggerTurn: true, deliverAs: "followUp" }), 0);
  });
};
...
const agent = new Agent({ initialState: { systemPrompt: "POC", model, thinkingLevel: "off", serviceTier: "auto", messages: [], tools: [] }, convertToLlm: () => [], streamFn: responseFor });
...
const postCompactionGoals = entries.filter((entry) => (entry.type === "custom" || entry.type === "custom_message") && entry.customType === "goal_context");
const postCompactionGoal = postCompactionGoals.length > 0;
const compactionCount = entries.filter((entry) => entry.type === "compaction").length;
const result = { cwd, calls, trace, entries: entries.map((e) => ({ type: e.type, customType: e.customType, summary: e.summary, firstKeptEntryId: e.firstKeptEntryId })), compactionApplied: Boolean(compaction), compactionCount, postCompactionGoalContext: postCompactionGoal, continuationCount: postCompactionGoals.length };
if (!compaction || compactionCount !== 2 || calls !== 2 || postCompactionGoals.length !== 1) {
