...
const localFactory = (await import(pathToFileURL(join(process.cwd(), "src/ralph-extension.js")).href)).createRalphExtension;
...
const loader = new DefaultResourceLoader({ cwd, agentDir, settingsManager: settings, extensionFactories: [localFactory({ enabled: true, bootstrap: "Read the durable Ralph plan." })], noSkills: true, noPromptTemplates: true, noThemes: true, noContextFiles: true, systemPrompt: "POC" });
await loader.reload();
...
if (!compaction || calls !== 2 || !postCompactionGoal) {
