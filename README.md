...
The public setup-discovery acceptance initializes both the source tree and a packed installation, then uses a spawned Prime Agent `0.9.3` RPC process with skills, prompt templates, themes, context files, and session persistence disabled to prove automatic project extension discovery, exact command-catalog exposure, packed-target binding, and abort acknowledgement. It does not control the host daemon or assert provider or canonical-skill behavior. It does not claim `/reset`, `/spec-it-out`, or `/plan` lifecycle behavior; the older mixed init/package fixtures remain compatibility and package evidence.
